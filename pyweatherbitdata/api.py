    ...
    async def update_forecast(self) -> None:
        ...
        try:
            ...
            entity_data = ForecastDescription(
                ...
                slp=self.cnv.pressure(base_data["slp"]),
                clouds=base_data["clouds"],
                wind_spd=self.cnv.windspeed(base_data["wind_spd"], self.homeassistant),
                wind_gust_spd=self.cnv.windspeed(base_data["wind_gust_spd"], self.homeassistant),
                wind_cdir=self.calc.wind_direction(base_data["wind_dir"]),
                wind_dir=base_data["wind_dir"],
                dewpt=self.cnv.temperature(base_data["dewpt"]),
                pop=base_data["pop"],
            ...
            for item in base_data:
                forecast_data = ForecastDetailDescription(
                    ...
                    clouds=item["clouds"],
                    wind_spd=self.cnv.windspeed(item["wind_spd"], self.homeassistant),
                    wind_gust_spd=self.cnv.windspeed(item["wind_gust_spd"], self.homeassistant),
